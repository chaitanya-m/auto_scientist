...
class BaseAgent:
    ...
    def select_action(self, accuracy_change_bin):
        # Select action using epsilon-greedy policy
        if accuracy_change_bin is None or np.random.rand() < self.epsilon:
            # If accuracy change bin is None, or with probability epsilon, return a random action
        ...
        elif np.amax(self.Q_table[accuracy_change_bin]) == np.amin(self.Q_table[accuracy_change_bin]):
            action_no = self.rng.integers(self.num_actions)
            ...
            action_no = np.argmax(self.Q_table[accuracy_change_bin])
